...
def show_login():
    ...
    with st.container():
        # HTML del selector de perfiles
        st.markdown(f"""
        <div class="login-header">
            <div style="transform:scale(0.5); transform-origin:center top;">{LOGO_SIDEBAR}</div>
            <h2 class="login-title">Selecciona tu perfil</h2>
            <p class="login-sub">Para acceder a la plataforma de convivencia escolar</p>
        </div>
        """, unsafe_allow_html=True)

        # Widgets de perfil (con botones invisibles de Streamlit encima)
        p1, p2, p3, p4 = st.columns(4)
        
        active_class = " pwidget-active"
        ...
        # Mostrar formulario si hay perfil seleccionado
        if panel != "select":
            st.markdown("---")
            c1, c2, c3 = st.columns([1, 1.5, 1])
            with c2:
                # Título del form basado en perfil
                names = {"colegio": "Docente / Directivo", "admin": "Administrador Animar", "familia": "Familia / Tutor", "alumno": "Alumno"}
                
                st.markdown(f"""
                <div class="login-form-wrap">
                    <h4 class="login-form-title">Ingresar como {names[panel]}</h4>
                    <p class="login-form-hint">Usa las credenciales demo proporcionadas.</p>
                </div>
                """, unsafe_allow_html=True)
                ...
                # Usar color de acento naranja para el botón
                st.markdown("""
                <style>
...
# ── Importar páginas de la carpeta /pages ─────────────────────────────────────
def get_pages():
    # Esta función simula la detección automática de Streamlit
    # Pero nos permite estructurar el menú interno post-login
    p = {
        "admin": [
        ...
        "teacher": [
            {"name": "Mis Aulas", "icon": "🚪"},
            {"name": "Sociograma", "icon": "LOGO_MOD"}, # Usar logo especial
    ...
    return p[st.session_state.user["role"]]

# ── Main post-login ───────────────────────────────────────────────────────────
def show_main():
    user = st.session_state.user
    
    # Sidebar oscura personalizada con Logo Animar Sidebar (blanco)
    ...
    # Navegación simulada basada en roles (esto lo hace st automáticamente en /pages)
    pages = get_pages()
    for p in pages:
        # Reemplazar ícono M3 por logo teal
    ...
    # Contenido principal simulado (Dashboard de Bienvenida)
    st.markdown(f"""
    ...
    # Contenido del Dashboard (Métricas y Alertas simuladas)
