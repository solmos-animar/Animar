...
def render():
        ...
        try:
            ...
            if not df_col_list.empty:
                col_destino = st.selectbox(
                    "Seleccionar Colegio para Docentes",
                ...
                with st.expander("➕ Agregar docente manualmente", expanded=False):
                    st.markdown("""
                        <div style="background:linear-gradient(135deg,#f0f4ff 0%,#e8f4f0 100%);
                            ...
                            padding:14px 18px 6px; margin-bottom:18px;">
                    ...
                    with st.form("form_docente_individual", clear_on_submit=True):
                        ...
                        apellido_ind = col_c.text_input("Apellido *", placeholder="Ej: García")
                        nombre_ind   = col_d.text_input("Nombre *",   placeholder="Ej: Lucía")
                        col_e, col_f = st.columns(2)
                        grado_ind    = col_e.selectbox("Grado *",    ["1°","2°","3°","4°","5°","6°","7°"])
                        division_ind = col_f.selectbox("División *", ["A","B","C","D","E"])
                        st.markdown("<div style='height:4px'></div>", unsafe_allow_html=True)
                        if st.form_submit_button("✅ Agregar Docente", type="primary", use_container_width=True):
                            if dni_ind and apellido_ind and nombre_ind:
                                ...
                                try:
                                    with conn.session as s:
                                        ...
                                        s.commit()
                                    st.success(f"✅ Docente **{apellido_ind.capitalize()}, {nombre_ind.capitalize()}** agregado a **{nombre_institucion}** ({grados_y_div_ind}).")
                                    st.rerun()
                                except Exception as e:
                                    st.error(f"Error al guardar: {e}")
                            else:
                    ...
                    if st.button("💾 Guardar cambios realizados en la tabla"):
                        with conn.session as s:
                            for _, r in df_editado.iterrows():
                                s.execute(text("UPDATE docentes SET apellido=:a, nombre=:n, grados_divisiones=:g, activo=:ac WHERE id=:id"),
    ...
    with tab3:
        ...
        try:
            ...
            archivo_al = st.file_uploader("Subir archivo Excel de alumnos con tutores", type=["xlsx"], key="uploader_alumnos")
            ...
            if archivo_al:
                ...
                if falt_al or falt_t1:
                    ...
                    df_raw['fecha_nacimiento'] = pd.to_datetime(df_raw['fecha_nacimiento'], dayfirst=True, errors='coerce').dt.date
                    fechas_mal = df_raw['fecha_nacimiento'].isna().sum()
                    sin_tutor1 = df_raw['tutor1_dni'].isna().sum()

                    st.markdown("**📋 Vista previa:**")
                    st.dataframe(df_raw[['dni','apellido','nombre','grados_y_div','fecha_nacimiento','tutor1_nombre','tutor1_apellido']].head(5), use_container_width=True, hide_index=True)

                    c1, c2, c3, c4 = st.columns(4)
                    c1.metric("Total alumnos", len(df_raw))
                    c2.metric("Colegio destino", nombre_colegio_al)
                    c3.metric("Fechas con error", fechas_mal, delta=None if fechas_mal==0 else "se guardan como nulas", delta_color="off" if fechas_mal==0 else "inverse")
                    c4.metric("Sin Tutor1", sin_tutor1, delta=None if sin_tutor1==0 else "filas serán rechazadas", delta_color="off" if sin_tutor1==0 else "inverse")

                    if sin_tutor1 > 0:
                        st.error(f"❌ {sin_tutor1} filas no tienen Tutor1. Corregí el Excel antes de continuar.")
                    else:
                        st.warning(f"⚠️ Estás por cargar **{len(df_raw)} alumnos** con sus tutores a **{nombre_colegio_al}**.")
                        if st.button("🚀 Confirmar y Guardar", type="primary", use_container_width=True):
                            ...
                            with conn.session as s:
                                for _, row in df_raw.iterrows():
                                    try:
                                        res_al = s.execute(text("""
                                            INSERT INTO alumnos (colegio_id, dni, apellido, nombre, grado, fecha_nacimiento)
                                            VALUES (:cid, :dni, :ape, :nom, :gd, :fn)
                                            ON CONFLICT (dni) DO UPDATE SET
                                                apellido=EXCLUDED.apellido, nombre=EXCLUDED.nombre,
                                                grado=EXCLUDED.grado, fecha_nacimiento=EXCLUDED.fecha_nacimiento
                                            RETURNING id
                                        """), {"cid": colegio_al, "dni": str(row['dni']).strip(),
                                        ...
                                        for pref in PREFIJOS:
                                            ...
                                            if col_dni_t not in df_raw.columns: continue
                                            dni_t = row.get(col_dni_t)
                                            if pd.isna(dni_t) or str(dni_t).strip() == "": continue
                                            ...
                                            s.execute(text("""
                                                INSERT INTO alumno_tutores (alumno_id, tutor_id, relacion, es_principal)
                                                VALUES (:aid, :tid, :rel, :esp)
                                                ON CONFLICT (alumno_id, tutor_id) DO UPDATE SET
                                                    relacion=EXCLUDED.relacion, es_principal=EXCLUDED.es_principal
                                            """), {"aid": alumno_id, "tid": tutor_id,
                                ...
                                s.commit()
                            if exitos: st.success(f"✅ Se procesaron **{exitos} alumnos** en {nombre_colegio_al}.")
                            if errores: st.error(f"❌ {errores} filas tuvieron errores.")
                            st.rerun()
            ...
            with st.expander("➕ Agregar alumno manualmente", expanded=False):
                st.markdown("""
                    <div style="background:linear-gradient(135deg,#f0f4ff 0%,#fef9f0 100%);
                        border-left:4px solid #d4580a; border-radius:0 12px 12px 0;
                        padding:14px 18px 8px; margin-bottom:20px;">
                        <span style="font-size:13px; color:#0f2240; font-weight:600;">📋 Datos del alumno</span><br>
                        <span style="font-size:12px; color:#5c5852;">Completá los datos del alumno y al menos un tutor para poder guardar.</span>
                    </div>
                """, unsafe_allow_html=True)
                ...
                with st.form("form_alumno_completo", clear_on_submit=True):
                    ...
                    dni_al = col_dni.text_input("DNI del alumno *", placeholder="Ej: 45123456")
                    ...
                    fecha_al    = col_fn.date_input("Fecha de nacimiento *", value=None,
                    ...
                    st.markdown("""
                        <div style="margin:24px 0 14px; padding:12px 18px;
                            ...
                            <span style="font-size:13px; color:#0f2240; font-weight:600;">👨‍👩‍👧 Tutores / Referentes</span><br>
                            <span style="font-size:12px; color:#5c5852;">Al menos un tutor es obligatorio. Podés cargar hasta 3.</span>
                        </div>
                    """, unsafe_allow_html=True)
                    ...
                    for i in range(st.session_state.n_tutores):
                        ...
                        tutores_data.append({"relacion":t_rel,"apellido":t_apellido,"nombre":t_nombre,
                                             "dni":t_dni,"telefono":t_tel,"email":t_email,"es_principal":t_principal})

                    col_add, col_save = st.columns([1,3])
                    agregar = col_add.form_submit_button(f"＋ Tutor {st.session_state.n_tutores+1}", disabled=st.session_state.n_tutores>=3)
                    guardar = col_save.form_submit_button("✅ Guardar Alumno con Tutores", type="primary", use_container_width=True)
                                    ...
                                    for t in tutores_data:
                                        ...
                                        s.execute(text("""
                                            ...
                                            ON CONFLICT (alumno_id, tutor_id) DO UPDATE SET
                                                relacion=EXCLUDED.relacion, es_principal=EXCLUDED.es_principal
                                        """), {"aid": alumno_id, "tid": tutor_id,
                                               "rel": t['relacion'], "esp": t['es_principal']})
                                    s.commit()
                                st.success(f"✅ **{apellido_al.capitalize()}, {nombre_al.capitalize()}** guardado/a con {len([t for t in tutores_data if t['dni'].strip()])} tutor/es.")
                                st.session_state.n_tutores = 1
                                st.rerun()
                            except Exception as e:
                                st.error(f"Error al guardar: {e}")
            ...
            with conn.session as s:
                res_padron = s.execute(text("""
                    SELECT a.id, a.dni, a.apellido, a.nombre, a.grado, a.fecha_nacimiento, a.activo,
                           COUNT(at2.tutor_id) AS cant_tutores,
                           STRING_AGG(at2.relacion || ': ' || t.apellido || ' ' || t.nombre ||
                               CASE WHEN at2.es_principal THEN ' ★' ELSE '' END,
                               ' | ' ORDER BY at2.es_principal DESC) AS tutores_resumen
                    FROM alumnos a
                    LEFT JOIN alumno_tutores at2 ON at2.alumno_id = a.id
                    LEFT JOIN tutores t ON t.id = at2.tutor_id
                    WHERE a.colegio_id = :cid
                    GROUP BY a.id, a.dni, a.apellido, a.nombre, a.grado, a.fecha_nacimiento, a.activo
            ...
            if not df_padron.empty:
                ...
                if sin_tutor > 0:
                    col_f2.markdown(f"<div style='padding-top:26px;'><span style='background:#fde8d0; color:#6b3000; padding:4px 10px; border-radius:20px; font-size:12px; font-weight:600;'>⚠️ {sin_tutor} sin tutor</span></div>", unsafe_allow_html=True)
                col_f3.markdown(f"<div style='padding-top:28px; font-size:13px; color:#5c5852;'>Total: <strong>{len(df_padron)}</strong> alumnos</div>", unsafe_allow_html=True)
                df_vista = df_padron if filtro_grado == "Todos" else df_padron[df_padron['grado'] == filtro_grado]
                st.dataframe(df_vista[['dni','apellido','nombre','grado','fecha_nacimiento','cant_tutores','tutores_resumen','activo']],
                    column_config={
    ...
    with tab4:
        ...
        with st.expander("➕ Crear nuevo usuario", expanded=True):
            ...
            with st.form("form_crear_usuario", clear_on_submit=True):
                ...
                nuevo_rol   = col2.selectbox("Rol *", options=ROLES, format_func=lambda x: ROLES_LABELS[x])

                col3, col4 = st.columns(2)
                nueva_pass  = col3.text_input("Contraseña *", type="password", placeholder="Mínimo 8 caracteres")
                nueva_pass2 = col4.text_input("Confirmar contraseña *", type="password", placeholder="Repetir contraseña")
                ...
                try:
                    ...
                    colegio_sel = st.selectbox(
                        ...
                        format_func=lambda x: "— Sin colegio —" if x is None else df_cols[df_cols['id']==x]['nombre'].iloc[0]
                    )
                except Exception:
                    colegio_sel = None
                ...
                if st.form_submit_button("✅ Crear Usuario", type="primary", use_container_width=True):
                    ...
                    if len(nueva_pass or "") < 8:  errores.append("Mínimo 8 caracteres")
                    ...
                    if errores:
                        ...
                        ok, msg = crear_usuario(conn, email=nuevo_email, password=nueva_pass,
                                                rol=nuevo_rol, colegio_id=colegio_sel)
                        if ok:
        ...
        filtro_rol    = col_f1.selectbox("Filtrar por rol", ["Todos"] + ROLES, format_func=lambda x: "Todos" if x=="Todos" else ROLES_LABELS[x])
        filtro_activo = col_f2.selectbox("Estado", ["Todos","Activos","Inactivos"])

        try:
            with conn.session as s:
                res = s.execute(text("""
                    SELECT u.id, u.email, u.rol, u.activo, u.creado_en, c.nombre AS colegio
                    FROM usuarios u
            ...
            if not df_u.empty:
                if filtro_rol != "Todos":    df_u = df_u[df_u['rol'] == filtro_rol]
                if filtro_activo == "Activos":   df_u = df_u[df_u['activo'] == True]
                elif filtro_activo == "Inactivos": df_u = df_u[df_u['activo'] == False]
                ...
                df_editado = st.data_editor(
                    ...
                    use_container_width=True, hide_index=True, key="edit_usuarios_table"
                )
                ...
                if st.button("💾 Guardar cambios de estado", use_container_width=True):
                    with conn.session as s:
                        for i, r in df_editado.iterrows():
                            uid = df_u.iloc[i]['id']
                            s.execute(text("UPDATE usuarios SET activo=:a WHERE id=:id"), {"a": r['activo'], "id": uid})
                        s.commit()
            ...
            with st.form("form_cambiar_pass", clear_on_submit=True):
                ...
                if st.form_submit_button("🔑 Cambiar contraseña", type="primary", use_container_width=True):
                    if nueva_p1 != nueva_p2:
                        ...
                        try:
                            with conn.session as s:
                                res = s.execute(text("SELECT id FROM usuarios WHERE email=:e"), {"e": email_reset.strip().lower()})
                                row = res.fetchone()
                            if not row:
